...
console.log('should exit here!');
	...
	server.put(routePrefix + '/user', ns.authorization.validateRequest, function(request, response) {
	...
	server.post(routePrefix + '/user', ns.authorization.validateRequest, function(request, response) {
		ns.logger.info('POST request=' + request.route.path);
		var userData = request.body;

		User.addUser(userData, function(err, data) {
			var result = { error: request.route.path + ' : ' + err };
			if (err) {
				ns.logger.error('error : %s, %s', request.route.path, err);
			} else {
	...
	server.del(routePrefix + '/user/:id', ns.authorization.validateRequest, function(request, response) {
